...
// ConnSocketConn implements the net.Conn interface on top of a qemu_pipe, communicating open,close events over the control socket.
...
// A Socket that uses one channel to control if a socket should be opend or closed.
...
// Opens the socket with the given virtual fd. The connection and errors will be posted back on the available channel.
...
// Opens a connection without blocking by calling accept and sending the identity message over the connection.
func (b *ControlSocket) open(fd uint32) (net.Conn, error) {
